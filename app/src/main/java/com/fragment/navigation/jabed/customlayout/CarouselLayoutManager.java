...
public class CarouselLayoutManager extends RecyclerView.LayoutManager {
    ...
    private final CarouselLayoutManager.LayoutHelper mLayoutHelper = new CarouselLayoutManager.LayoutHelper(MAX_VISIBLE_ITEMS);

    private CarouselLayoutManager.PostLayoutListener mViewPostLayout;

    private final List<CarouselLayoutManager.OnCenterItemSelectionListener> mOnCenterItemSelectionListeners = new ArrayList<>();
    private int mCenterItemPosition = INVALID_POSITION;
    private int mItemsCount;

    private CarouselLayoutManager.CarouselSavedState mPendingCarouselSavedState;
     ...
     */
    @SuppressWarnings("unused")
    public void setPostLayoutListener(@Nullable final CarouselLayoutManager.PostLayoutListener postLayoutListener) {
    ...
    /**
     * @param onCenterItemSelectionListener listener that was previously added by {@link #addOnItemSelectionListener(CarouselLayoutManager.OnCenterItemSelectionListener)}
     */
    public void removeOnItemSelectionListener(@NonNull final CarouselLayoutManager.OnCenterItemSelectionListener onCenterItemSelectionListener) {
    ...
    public void smoothScrollToPosition(@NonNull final RecyclerView recyclerView, @NonNull final RecyclerView.State state, final int position) {
        final float MILLISECONDS_PER_INCH = 140f;
        final CarouselSmoothScroller mySmoothScroller =
                new CarouselSmoothScroller(recyclerView.getContext()) {
                    ...
                    public PointF computeScrollVectorForPosition(final int targetPosition) {
                        ...
                        /*if(targetPosition<position&&targetPosition>position-4){
                        }*/
                        Log.e("positions", position + " target: " + targetPosition);
    ...
    protected PointF computeScrollVectorForPositionNew(final int targetPosition) {
        if (0 == getChildCount()) {
            return null;
        }
        final float currentScrollPosition = makeScrollPositionInRange0ToCount(getCurrentScrollPosition(), mItemsCount);
    ...
    private void selectItemCenterPosition(final int centerItem) {
        for (final CarouselLayoutManager.OnCenterItemSelectionListener onCenterItemSelectionListener : mOnCenterItemSelectionListeners) {
    ...
    private void fillDataVertical(final RecyclerView.Recycler recycler, final int width, final int height, final boolean childMeasuringNeeded) {
        ...
        for (int i = 0, count = mLayoutHelper.mLayoutOrder.length; i < count; ++i) {
            final CarouselLayoutManager.LayoutOrder layoutOrder = mLayoutHelper.mLayoutOrder[i];
    ...
    private void removeAndRecycleUnusedViews(final CarouselLayoutManager.LayoutHelper layoutHelper, final RecyclerView.Recycler recycler) {
    ...
    private void fillChildItem(final int start, final int top, final int end, final int bottom, @NonNull final CarouselLayoutManager.LayoutOrder layoutOrder, @NonNull final RecyclerView.Recycler recycler, final int i, final boolean childMeasuringNeeded) {
    ...
    public Parcelable onSaveInstanceState() {
        if (null != mPendingCarouselSavedState) {
            return new CarouselLayoutManager.CarouselSavedState(mPendingCarouselSavedState);
        }
        final CarouselLayoutManager.CarouselSavedState savedState = new CarouselLayoutManager.CarouselSavedState(super.onSaveInstanceState());
    ...
    public void onRestoreInstanceState(final Parcelable state) {
        if (state instanceof CarouselLayoutManager.CarouselSavedState) {
            mPendingCarouselSavedState = (CarouselLayoutManager.CarouselSavedState) state;
    ...
    private static class LayoutHelper {
        ...
        private CarouselLayoutManager.LayoutOrder[] mLayoutOrder;

        private final List<WeakReference<CarouselLayoutManager.LayoutOrder>> mReusedItems = new ArrayList<>();
        ...
        public void initLayoutOrder(final int layoutCount) {
            if (null == mLayoutOrder || mLayoutOrder.length != layoutCount) {
                ...
                mLayoutOrder = new CarouselLayoutManager.LayoutOrder[layoutCount];
                fillLayoutOrder();
            }
        }
        ...
        public void setLayoutOrder(final int arrayPosition, final int itemAdapterPosition, final float itemPositionDiff) {
            final CarouselLayoutManager.LayoutOrder item = mLayoutOrder[arrayPosition];
        ...
        public boolean hasAdapterPosition(final int adapterPosition) {
            if (null != mLayoutOrder) {
                for (final CarouselLayoutManager.LayoutOrder layoutOrder : mLayoutOrder) {
        ...
        private void recycleItems(@NonNull final CarouselLayoutManager.LayoutOrder... layoutOrders) {
            for (final CarouselLayoutManager.LayoutOrder layoutOrder : layoutOrders) {
        ...
        private CarouselLayoutManager.LayoutOrder createLayoutOrder() {
            final Iterator<WeakReference<LayoutOrder>> iterator = mReusedItems.iterator();
            while (iterator.hasNext()) {
                final WeakReference<CarouselLayoutManager.LayoutOrder> layoutOrderWeakReference = iterator.next();
                final CarouselLayoutManager.LayoutOrder layoutOrder = layoutOrderWeakReference.get();
                iterator.remove();
                if (null != layoutOrder) {
                    return layoutOrder;
                }
            }
            return new CarouselLayoutManager.LayoutOrder();
        }
    }
    ...
    public static class CarouselSavedState implements Parcelable {
        ...
        public CarouselSavedState(@NonNull final CarouselLayoutManager.CarouselSavedState other) {
        ...
        public static final Parcelable.Creator<CarouselLayoutManager.CarouselSavedState> CREATOR
                = new Parcelable.Creator<CarouselLayoutManager.CarouselSavedState>() {
            @Override
            public CarouselLayoutManager.CarouselSavedState createFromParcel(final Parcel source) {
                return new CarouselLayoutManager.CarouselSavedState(source);
            }

            @Override
            public CarouselLayoutManager.CarouselSavedState[] newArray(final int size) {
                return new CarouselLayoutManager.CarouselSavedState[size];
            }
        };
    }
}